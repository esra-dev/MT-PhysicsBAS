#!/usr/bin/env python3
"""Rebuild the corrected Phase-4 registered tables from committed archives.

Validates every corrected run archive (TRAINING_OK gates, schedule identity,
v2 benchmark schema), reruns the frozen registered analysis over ALL run
archives together (the campaign is two seed-half dispatches), and compares
the result against the committed CSVs using the 12-significant-digit
canonical numeric serialization (helper copied from the Phase-1 pattern;
Phase-1 analysis files stay byte-frozen).
...
try:
    from analysis import phase4_v2_registered_family as family
    from analysis import validate_phase4_v2_archive as archive_validator
except ImportError:  # executed as a script from the repo root
    import phase4_v2_registered_family as family
    import validate_phase4_v2_archive as archive_validator


FLOAT_TEXT = re.compile(r"[-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?")
TABLES = ("phase4_v2_registered_family.csv", "phase4_v2_ladder_trend.csv")
...
def main() -> int:
    ...
    parser.add_argument("campaign_root", type=Path, nargs="?",
                        default=Path("phase4_v2_corrected"))
    ...
    for run_dir in run_dirs:
        ...
        errors = archive_validator.validate(run_dir, expected_seeds=seeds)
        if errors:
            print(f"{run_dir}: archive validation FAILED:", file=sys.stderr)
            for error in errors:
                print(f"- {error}", file=sys.stderr)
            return 1
        print(f"{run_dir}: archive gates pass ({len(seeds)} seeds x 4 labs).")

    with tempfile.TemporaryDirectory(prefix="phase4-v2-reproduce-") as tmp:
        rebuilt = Path(tmp)
        family.run(run_dirs, rebuilt)
        committed = root / "analysis" / "registered"
        for name in TABLES:
            _compare(committed / name, rebuilt / name)
    print("All corrected Phase-4 canonical numeric CSVs are byte-equivalent.")
    return 0
