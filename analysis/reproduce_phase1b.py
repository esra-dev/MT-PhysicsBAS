...
Mirrors ``analysis/reproduce_phase1_v2.py``: discover the four run archives
under a campaign directory (exactly one per Phase-1b mode; duplicates fatal),
validate each with the confirmatory-stage Phase-1b gates, verify the SHA-256
inventory when one is present, re-run ``analysis/phase1b_report.py`` over the
four mode roots into a scratch directory, and canonically byte-compare the
rebuilt ``phase1b_registered_family.csv`` and ``phase1b_supporting.csv``
against the committed tables in ``<campaign>/analysis/registered/``.
"""
...
def reproduce(campaign: Path, work: Path,
    ...
    seeds_by_mode: dict[str, list[int]] = {}
    for mode, archive in sorted(archives.items()):
        ...
        seeds_by_mode[mode] = _workflow_seeds(archive)
    ...
    if len({tuple(seeds) for seeds in seeds_by_mode.values()}) != 1:
