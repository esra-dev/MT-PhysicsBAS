...
## 0. TL;DR for the meeting (read this first)
...
The thesis arc the advisors agreed on:
...
### 2.3 Energy efficiency (the "side effect" the advisors flagged)
...
### 3.1 The blinds model (what the advisors asked to see)
...
### 3.3 The other stereotypes (one line each)
...
## 5. Demos ready for the meeting

Two complementary demos. Full how-to in [docs/DEMO_GUIDE.md](DEMO_GUIDE.md).

### Demo A — Agent Decision Explainer (terminal, exact data, no setup)

A Python script that reads a **real benchmark step-log** and narrates, step by step:
the scenario, the goal (per-zone target ranks), the current outdoor-sun rank, the action the
agent chose, **why** (which stereotype/prior was in play), what the simulator did, and whether the
goal was reached. It renders a side-by-side **`ql_true` vs `ql_false`** walkthrough so the
difference in behaviour is visible.

- Script: [analysis/demo_decision_explainer.py](../analysis/demo_decision_explainer.py)
- Pre-rendered transcript (so you can present even with no laptop): [docs/demo_walkthrough_custom9.md](demo_walkthrough_custom9.md)

**The scenario & goal (what to say):** "Four office zones. Targets are `Z1=bright(3), Z2=medium(2), Z3=bright(3), Z4=medium(2)`. Outdoor sunlight varies. The agent has 20 steps to toggle lights/blinds/corridor light to hit all four targets at once. We watch the stereotype agent prefer the **blind** when the sun is high (energy-free daylight harvesting) and a **task light** when it is dark — and we watch the naive agent flail."

### Demo B — Agent Trace Cockpit (visual dashboard)

A React/Vite dashboard that **replays a real benchmark trace** with a 4-zone lab view, the agent's
per-step decision, the ontology's prediction vs the observed effect, and a weakness detector that
lights up when an unmodelled effect fires.
...
Then **load demo (custom9)** (a real `ql_true` benchmark trace I wired in) or **load .jsonl** to open
any trace from `tmp_sweep_n10_results/benchmark/results_seed*/...`.

---

## 6. The simple-lab track (closing the "show it works in a simple lab" gap)
...
**Decision to take to the advisors:** approve the simple-lab track as the *first* result chapter
("mechanism works in a clean, convergent setting"), with the existing n=10 weakness-lab results as
the *stress-test* chapter.
...
## 8. Three-week achievement log (since the 22 May meeting)
...
## 11. Anticipated advisor questions & crisp answers

- **"Did the hypothesis hold?"** Partially and significantly: yes on speed+goal-rate for 2/3 labs, yes on energy for 3/3; no on speed for the headline lab, for a now-understood, fixable reason.
- **"Is the custom9 failure a conceptual problem you can fix?"** Yes, and we now know the exact
  mechanism (§4.1, from the existing data): the blind/sun prior is **strongly beneficial at bright
  sun** (Δ=+0.280) but **mis-fires at medium sun** (Δ=−0.157) because its IV gate (`ivMinRank=1`)
  recommends blinds two ranks too early. The fix is a one-line `ivMinRank=3` ontology change; the
  base learner's non-convergence in 20 steps (§4.2) is a separate, structural factor. The earlier
  "training sun-coverage gap" story was disproven. Neither factor refutes the hypothesis.
- **"Is energy a real result or an artefact?"** Real and maximally significant, but honestly framed as an **emergent side effect of the priors** (no energy term in the reward).
- **"How does the blind model handle outdoor light?"** It's a **Mediates** mechanism: the blind gates the sun→indoor-light transfer; the sign is derived per-state from the sun rank and *learned* end-to-end, not hard-coded (§3).
- **"Did you test the naive versions?"** Yes — `ql_false` (uninformed learner) and `rule_based` (expert) are run on every lab/seed; the expert still wins on the hard labs, which we report honestly.
- **"What's the demo?"** Four office zones, per-zone brightness targets, varying sun, 20 steps to satisfy all targets — we watch (and explain) each agent decision (§5).

---

## 12. Loose end to check before the meeting

The last terminal command
`gh workflow run sweep-paper.yml -f profiles=custom9 -f run_mode=paper_h40 -f publish_results=false`
returned **exit code 1** — the horizon-ablation run may not have launched. Re-check with
`gh run list --workflow=sweep-paper.yml -L 5` and re-dispatch if needed before claiming it is running.
