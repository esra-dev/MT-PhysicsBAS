...
**Q4 · Are we done with Phase 3?** **Yes — done.** Every Phase 3 objective in the master doc is delivered and demonstrated in CI: delay learning, KG writeback, and time-bounded exploitation. The **delay-learning** headline is statistically sound as registered (real replica variance; bootstrap p = 0.000, q_BH = 0.000, Cliff's δ = 1.0). The **deadline-compliance** headline is a **worked demonstration, not statistical inference**: `6/6 vs 3/6` deadline compliance, identical across all 10 replicas of the confirmatory sweep (run `27621106006`, §19.4) — the planner is deterministic given the learned delay table, so replicas are copies, not independent samples, and no compliance p-value at any n (including the mechanical n = 10 signed-rank floor `2/2¹⁰ = 0.00195`) carries inferential weight (§19.4 headline note). The n = 10 default is now the standing configuration (§19.1).

**Recommendation: Phase 3 is complete.** Delay learning to ≤ ~1.8 % rel err is confirmed with statistical significance across two labs and 10 replicas; time-bounded compliance (`6/6 vs 3/6`) is demonstrated deterministically across the same matrix, reported as counts. No further runs are required for the write-up.
...
- **n = 5 Wilcoxon floor (0.0625) — floor artefact removed; the number remains non-inferential.** The n = 5 sweep reported `p_wilcoxon = 0.0625` purely because the two-sided signed-rank floor at n = 5 is `2 / 2⁵ = 0.0625`. The confirmatory **n = 10** sweep (§19.4) lengthens the vector so the floor drops to `2 / 2¹⁰ = 0.00195`, with identical point estimates — but because the planner is deterministic the ten replicas carry the information of one, so **no compliance p-value at any n is evidence of an effect**; the compliance result is reported as the count-based demonstration `6/6 vs 3/6` (§19.4 headline note). This bullet is retained as a record of why n was raised, mirroring how Phase 2 standardised on n ≥ 6 for signed-rank.
...
The two-sided Wilcoxon signed-rank test on n paired, all-positive differences has a hard minimum p-value of `2 / 2ⁿ`. At n = 5 that floor is **0.0625 > 0.05**, so the §16.3 Wilcoxon column reads `0.0625` *no matter how large the effect* — a pure sample-size artefact, not weak evidence. (For the deterministic compliance metrics no p-value at any n — Wilcoxon or bootstrap — is inferential; the authoritative reading is the §19.4 count-based demonstration. The delay-estimate metrics, which have real replica variance, are where bootstrap p = 0.000, q_BH = 0.000 and Cliff's δ = 1.0 carry inferential weight.) The floor drops below 0.05 as soon as **n ≥ 6** (`2/2⁶ = 0.03125`); at the Phase 2 default of **n = 10** it is `2/2¹⁰ ≈ 0.00195`.
...
**The headline result (worked demonstration, not statistical inference): `6/6 vs 3/6` deadline compliance (ql_true meets all six goals; ql_false meets three), identical across all 10 replicas (deterministic planner → replicas are copies, not independent samples).** The tabulated `p_wilcoxon = 0.001953125` is not evidence of an effect: it is the mechanical `2 / 2¹⁰` two-sided signed-rank floor, and because the planner is deterministic the ten replicas carry the information of one — so it must be read as a count-based demonstration, not a significance test. Every other statistic is bit-identical to the n = 5 run (§16.3): diff `+1.0` / `+0.5`, CI `[1.0,1.0]` / `[0.5,0.5]`, bootstrap p = 0.000, Cliff's δ = 1.0, q_BH = 0.000 — again a consequence of determinism, since adding replicas did not move a single point estimate.
