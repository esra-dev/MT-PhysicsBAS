...
via `analysis/reproduce_phase1b.py`.
...
| M5 labband extended−baseline deviation (EXPLORATORY) | −0.1625 | [−0.334, −0.001] | 0.078 | — | predicted direction, not significant (as its 0.13–0.24 power predicted) |
| M6 chain3 frozen−baseline RMST | −0.5 presentations | [−0.64, −0.36] | 7.6e-6 | 1.9e-5 | **significant, ~37× below the SESOI** (−5% of horizon) |

Censoring was zero in every (scenario × seed × arm) cell, so M6's endpoint is
RMST. Rank-biserial was ±1 for M1/M3/M6 (every seed in the predicted
direction).
...
3. **M5 (exploratory)** moved in the predicted direction (extended reduced
   band deviation vs baseline) without reaching significance; per protocol
   this is not evidence of no effect, and it is the one hint the direction
   channel does something on the deviation margin.
...
- Registration §4 glossed M6's horizon as "H = 3000/8 = 375" using the
  ...
  censoring the horizon is inert — the RMST values are identical under
  either H — but the arithmetic slip in the registration text is recorded
  here.
...
deterministic `PolicyEnergyCost`, overshoot proxy, and cycling. Headline
observations (descriptive, uncorrected): the redundancy-only analogue of the
relevance K-slope is null (−2.4e-6, p≈0.46) — the tiny M1 slope is genuinely
knowledge-layer, NOT redundancy mimicry — while redundancy reproduces
roughly 60% of the fragmentation DiD (+7.8e-4 of +1.3e-3) and roughly 45%
of the chain RMST speed-up (−0.225 of −0.5), extending the Phase-1
partial-mimicry pattern to those two members.
