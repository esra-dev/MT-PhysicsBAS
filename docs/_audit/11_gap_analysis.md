...
| REQ-28 (recognize unexpected behaviour of policy action) | `observeForFaults` after every adapt step: dispatched action's KG prediction vs observed transition, DEAD/INVERTED verdicts (QLearner.java#L1078-L1253, 02 §9; wired in `@do_step_adapt`, 03 §1.4) | Detection recall 100 % in all 18 lamp cells (§20.6.2) and all 4 blind cells (§21.7.1); DetectEpisode ≈ 0 both arms (§20.6.1) | **KEEP** | **WITHDRAWN 2026-07-22: recall/precision verification is protocol-affected — the runs of record contain healthy-component blacklists (all lab3 single-lamp cells, every replica, both arms) and 5/10 missed second-fault detections in lab3_f2dead_lowsun KG. See `docs/PHASE2_PROTOCOL_AFFECTED_NOTICE_2026-07-22.md` item 3.** |
| REQ-29 (recheck with physics knowledge) | The detector *is* the physics recheck: per-step comparison against the KG-derived claim set (falsifiable zones, IV gates, structural guards) (02 §9 detection paragraph) | Structural guards (multi-zone feeder spared, IV-gated blinds adjudicated only under sun ≥2, contaminated zones skipped) produce 0 false positives on all confirmatory cells (§20.6.2, §21.7.4 Tier-1: 10/10 clean) | **KEEP** | **WITHDRAWN 2026-07-22: "0 false positives" and "Tier-1: 10/10 clean" are contradicted by the committed runs of record — SecondaryDetectEpisode ≥ 0 in every replica of lab3_f1dead/f1inv/f1dead_z2/f1inv_z2 (both arms) and 4/10 lab2_f1bdead vanilla replicas (0/10 KG; treatment-correlated), incl. one wrong primary. The suspect-gated abstain does not cover healthy-co-feeder rank masking. See `docs/PHASE2_PROTOCOL_AFFECTED_NOTICE_2026-07-22.md` item 3.** |
