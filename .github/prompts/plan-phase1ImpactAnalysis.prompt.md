# Phase 1 Impact Analysis — Thesis Pivot
...
The `custom9s` lab built on 2026-06-08 (clean 2-zone, blind↔sun mediation, converges in ~1000 eps) is already aligned with this new direction — it's effectively a working draft of "Lab_Medium" / "Lab_Complex".

---

## Phase 1 Impact Analysis — What to Modify / Create / Deprecate

### A. Node-RED simulator flows (the ground-truth physics)

| Lab | File | Action | Notes |
|---|---|---|---|
| `Lab_Simple` (1 zone, 1 lamp, 1 sensor) | `simulator/simulator_flow_simple.json` | **CREATE** (new) | New port (e.g. 1892). Must include `/health` endpoint (preflight gotcha — see `custom9s-simple-lab-build.md`). Single TaskLight, single zone lux node, single illuminance sensor. No blinds, no spotlight, no sun coupling. |
...
| `src/resources/lab-ontology-custom2.ttl`, `wot-mappings-custom2.ttl` | **NOT NEEDED for Phase 1** (they're the 4-zone schema for the weakness labs). Keep on disk. |
...
### H. What gets formally deprecated (but **not deleted**) for Phase 1

- Weakness labs `custom2..custom7` (flows, `.node-red-customX/` dirs, scenario JSONs, `lab_profile/13` records) — held for Phase 2.
- `custom8`, `custom9` — held pending re-classification (the audit notes flag them as marginal-IV; they may or may not be a useful "Lab_Complex_v2").
...
1. **Lab_Complex source:** Do you want to (a) **promote `custom9s` as-is** to be `Lab_Complex` (zero new simulator work — only registry/config aliasing) or (b) **fork it into a new `simulator_flow_complex.json`** with a fresh name, leaving `custom9s` untouched in history? (a)
2. **Accelerator features on the KG-primed arm:** The master doc says "KG accelerates learning in clean environments." Three existing accelerators are policy-invariant: (i) the stereotype Q-init bias (the core KG prior), (ii) PBRS reward shaping (`reward_shaping: pbrs`), (iii) adaptive trust calibration. Should Phase 1 turn **all three** ON for the `ql_true` arm, or only the core KG prior (i)? Turning on (ii) and (iii) muddies the "what is the KG contributing?" attribution; turning them off makes the comparison cleaner but slower.